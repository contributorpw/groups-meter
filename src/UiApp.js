function uiApp_() {
    var app = UiApp.createApplication().setTitle('Gmail Meter - Forum version');
    ...
    var grid = app.createGrid(3, 1).setCellSpacing(5).setId('grid');
    grid.setWidget(0, 0, app.createLabel('Step 1: Indicate the Group Email Address:'));
    grid.setWidget(1, 0, app.createTextBox().setName('groupEmailAddress'));
    var button = app.createButton('Step 2 >>', app.createServerHandler('selectReportType_').addCallbackElement(mainPanel));
    button.addClickHandler(app.createClientHandler().forEventSource().setEnabled(false).forTargets(imageOnLoad).setVisible(true));
...
function selectReportType_(e){
  var app = UiApp.getActiveApplication();
  var mainPanel = app.getElementById('mainPanel');
  mainPanel.add(app.createHidden('groupEmailAddress', e.parameter.groupEmailAddress));
  var imageOnLoad = app.getElementById('imageOnLoad').setVisible(false);
  var grid = app.getElementById('grid').clear().resize(4, 2).setCellSpacing(35);
  ...
  var buttonChoice1 = app.createButton('Get it', app.createServerHandler('getMonthlyReport_').addCallbackElement(mainPanel));
...
function getMonthlyReport_(e) {
    var app = UiApp.getActiveApplication();
    init_(e.parameter.groupEmailAddress);
    getReport_(app);
    return app;
}
...
function getCustomReport_() {
    ...
    var serverHandler = app.createServerHandler('checkAndStart_').addCallbackElement(mainPanel);
...
function checkAndStart_(e) {
    ...
    if (timeRange > 0) {
        ...
        init_(e.parameter.groupEmailAddress, e.parameter.sDate, e.parameter.eDate);
        getReport_(app);
    }
    else {
